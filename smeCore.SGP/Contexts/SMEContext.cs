...
namespace smeCore.SGP.Contexts
{
  public class SMEContext : DbContext
  {
    ...
    public DbSet<smeCore.Models.Academic.SchoolTerm> SchoolTerms { get; set; }
    #endregion -------------------- ACADEMIC --------------------
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
      ...
      var Code1 = new smeCore.Models.Organization.Code();
      Code1.NewID();

      modelBuilder.Entity<smeCore.Models.Organization.Code>().HasData(new
      {
        Code1.Id,
        Name = "Código EOL",
      });
      ...
      for (int k = 0; k <= 5; k++)
      {
        ...
        var student1 = new smeCore.Models.Academic.Student();
        student1.NewID();


        modelBuilder.Entity<smeCore.Models.Academic.Student>().HasData(new
        {
          student1.Id,
        });

        var studentCode1 = new smeCore.Models.Organization.StudentCode();
        studentCode1.NewID();

        modelBuilder.Entity<smeCore.Models.Organization.StudentCode>().HasData(new
        {
          studentCode1.Id,
          Value = codigoEol.ToString(),
          CodeId = Code1.Id,
        ...
        if (codigoEol == 10)
        {
          for (int j = 0; j < 1; j++)
          {
            var absence = new smeCore.Models.Academic.ClassAbsence();
            absence.NewID();
            modelBuilder.Entity<smeCore.Models.Academic.ClassAbsence>().HasData(new
            {
              absence.Id,
              Date = DateTime.Now,
              CodeEol = codigoEol.ToString(),
              studentCodeId = studentCode1,
            });
          }
        }
